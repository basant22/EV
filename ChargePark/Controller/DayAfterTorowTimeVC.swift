...
class DayAfterTorowTimeVC: UIViewController {
...
//        layout.headerReferenceSize = CGSize(width: self.collView.frame.size.width, height: 100)
        ...
        lblHeading.text = "Slot-Status on " +  Date().add(Day: 2).toddMMMyyString()
    ...
    @IBAction func doneVC(_sender:UIButton){
        ...
        if let selData = self.arrSelectedData.first,let timeStart = selData.0.components(separatedBy: "-").first{
           ...
           // let dateTim = timeStart.toHHmmssSSSZTime().toHHmmssSSSZ()
            ...
            //let dt = dateTime + timeStart
            if let vc = tbvc{
                vc.selectionComplete(self.arrSelectedIndex,dtt,"",vc)
            }
        }else{
            if let vc = tbvc{
...
extension DayAfterTorowTimeVC:UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
